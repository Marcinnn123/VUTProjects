...
int nextIndex( int index ) {
	...
	return (index + 1) % QUEUE_SIZE; // pokud je na konci pole, bude 0
}
...
int Queue_IsFull( const Queue *queue ) {
	return ((queue->firstIndex == nextIndex(queue->freeIndex)) || ((queue->firstIndex == 0) && (queue->freeIndex == QUEUE_SIZE-1))); // 2 cast je kontrola kruhovosti 
}
...
void Queue_Remove( Queue *queue ) {
	...
	queue->firstIndex = nextIndex(queue->firstIndex); // posuneme first index o jeden dal
	if (queue->firstIndex == QUEUE_SIZE){ // kontrola kruhovosti pole
		queue->firstIndex = 0;
	}
}
/**
...
void Queue_Dequeue( Queue *queue, char *dataPtr ) {
		...
		Queue_Front(queue, dataPtr); // vrati hodnotu prvniho prvku
		Queue_Remove(queue); // odstrani prvni prvek
}
...
void Queue_Enqueue( Queue *queue, char data ) {
	...
	queue->array[queue->freeIndex] = data; // vlozi data na nasledujici misto v fronte
	queue->freeIndex = nextIndex(queue->freeIndex); // posune volne misto dal
